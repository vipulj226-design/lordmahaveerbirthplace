...
export default function Header() {
  return (
    <header
      className="header-wrapper fixed top-0 left-0 right-0 z-[1000] w-full bg-neutral/96 backdrop-blur"
      style={{
    ...
    >
      ...
      <nav className="hidden md:flex justify-between items-center px-6 py-4 max-w-[1400px] mx-auto">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-heading text-[1.1rem] font-bold text-primary">
            ☸ Lord
            <span className="text-accent-alt">Mahaveer</span> Birthplace
          </span>
        </div>
        ...
        <ul className="flex gap-1">
          {navigationLinks.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                className="nav-link px-3 py-1.5 text-xs uppercase font-paragraph rounded transition-smooth text-primary hover:bg-primary hover:text-accent-alt"
              >
                {link.label}
              </a>
            </li>
      ...
      <div className="md:hidden flex flex-col">
        {/* Mobile Logo */}
        <div className="flex items-center justify-center gap-2 py-3 border-b border-accent-lighter">
          <Image
        ...
        <div
          className="grid gap-0 border-t border-accent-lighter"
          style={{
        ...
        >
          {navigationLinks.map((link, index) => (
            <a
              key={link.href}
              href={link.href}
              className="mobile-nav-link flex flex-col items-center justify-center min-h-[44px] text-[0.58rem] uppercase font-paragraph text-primary transition-smooth hover:bg-primary hover:text-accent-alt"
              style={{
